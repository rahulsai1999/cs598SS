...
def plot_disk_throughput(hardware_type, db_type):
    num_threads = [2, 3, 4, 5, 6]
    disk_throughput_lst = []
    actual_throughput_lst = []
    node = "node1" if hardware_type == "nvme" else "node-0"
    for num_thread in num_threads:
        csv_file_name = f"data/final/{hardware_type}/{db_type}/outWorkloadA.txt{num_thread}-{node}-20230511.tab"
